public class LinkedList {
    ...
    private boolean isInIncreasingOrder(Item start) {
        //if it's reached or is the last item, return true
        if (start == null ||start.next == null) {
            return true;
        }
        //compare the current with the next. If it's in increasing order, recur the function
        if (start.data < start.next.data) {
            return isInIncreasingOrder(start.next);
        }
        //if it's not the last item, nor is smaller than the next item, return false
        return false;
    }
